...
async fn при_чужом_отпечатке_учётные_данные_не_отправляются() {
    ...
    let attempts = server
        .exec_inside("grep -c 'Failed password' /var/log/auth.log 2>/dev/null || echo 0")
        .unwrap_or_else(|_| String::from("0"));
    assert_eq!(
        attempts.trim(),
        "0",
        "на сервер ушла попытка входа, хотя отпечаток не совпал"
    );
}
...
async fn много_каналов_в_одном_соединении() {
    ...
    // Сервер должен видеть одно соединение, а не двенадцать.
    let established = server
        .exec_inside("ss -tn state established '( sport = :22 )' | tail -n +2 | wc -l")
        .unwrap_or_else(|_| String::from("1"));
    let count: usize = established.trim().parse().unwrap_or(1);
    assert!(
        count <= 2,
        "открыто {count} соединений вместо одного — мультиплексирование не работает"
    );
}
...
async fn обрыв_соединения_замечается() {
    ...
    // Команда обязана завершиться ошибкой, а не зависнуть навсегда.
    let result =
        tokio::time::timeout(std::time::Duration::from_secs(20), conn.exec("echo жив")).await;

    match result {
        Ok(Ok(out)) if out.ok() => panic!("команда выполнилась на мёртвом соединении"),
        Ok(_) | Err(_) => {}
    }
}
