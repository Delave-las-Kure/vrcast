...
pub mod api {
    ...
    /// Сколько соединений веб-сервер обслуживает прямо сейчас (FR-019a, веха A).
    ///
    /// Это факт наличия раздачи, а не число зрителей конкретного файла: таблица
    /// соединений не говорит, что качают. Неудача подсчёта — ноль: предупреждение
    /// полезно, но ради него нельзя отказать в удалении.
    async fn active_connections(conn: &Connection) -> usize {
        let cmd = "ss -tn state established '( sport = :443 or sport = :80 )' 2>/dev/null | tail -n +2 | wc -l";
        conn.exec(cmd)
            .await
            .ok()
            .filter(|o| o.ok())
            .and_then(|o| o.trimmed().parse::<usize>().ok())
            .unwrap_or(0)
    }
